...
- **Don't auto-exclude on a lone Apollo signal — require corroboration.** `suitability_status` is currently **annotate-only**; nothing downstream excludes on it. Before any exclusion logic is built, do **not** auto-drop a candidate on a single `rejected_unsuitable`: Apollo can confidently resolve the **wrong org** from a discovered domain and return a self-consistent but wrong verdict. (Observed live: `ibtinc.com` — Tavily vendor "IBT Industrial Solutions", a US/Kansas distributor — resolved in Apollo to a Lahore, Pakistan "professional training & coaching" company, with country *and* state *and* address *and* industry all consistently Pakistani, so the rejection looked authoritative but the org was wrong.) Keep `rejected_unsuitable` human-reviewable (annotate-don't-remove), or gate auto-exclusion on **corroboration** — e.g. non-US **and** requirement-match reject and/or below the suitability floor. The candidate carries `apollo_country`/`apollo_industry` so a human can see *why* it was flagged. Note also that `suitability_status` and the pre-existing `suitability_floor` (which already sets `rejection_reason`) are independent signals that can disagree (an Apollo-`confirmed` US supplier can still be dropped by a sub-floor `suitability_score`) — reconcile them when defining exclusion.
- **Wrong-org resolution cuts BOTH ways — gate the rescue on name consistency.** The same domain→org mismatch that makes a lone Apollo *reject* untrustworthy (IBT, above) also makes a *rescue* untrustworthy: a floor-rejected candidate can be rescued on a `confirmed` verdict that actually belongs to a **different** org Apollo resolved from a mis-attributed domain (observed: J&D Manufacturing rescued via `qcsupply.com`'s verdict — benign there, but the mechanism is wrong). So the suitability reconciliation **gates the auto-rescue on a name-consistency check** (`_names_plausibly_match`: discovery vendor name vs Apollo's resolved `apollo_org_name`, lenient — suffix/variant tolerant, catches gross mismatches only). Confirmed-but-name-mismatch (or missing org name) → **withhold** the rescue (leave the floor reject), annotate `rescue_withheld_name_mismatch`. This requires Apollo's org name to be persisted (`apollo_org_name`) so cache-hit candidates can be checked too. The reject path stays flag-only (already safe). Rescue/reject still **remove nothing** — exclusion remains a separate, not-yet-built step.
