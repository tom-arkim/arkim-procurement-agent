...
| **Auth-dependent cluster — H1 routing / M1 / D2 (Phase R)** | The `_execute` phase guard is **in** (Phase R, commit `20df65c`): an order places only from an `approved`/`executing` run, closing H1's select→execute bypass. **(a) dual-approver routing (H1) — LANDED (auth-independent half):** the FastAPI `select-candidate` now persists `_approval_path` via `determine_approval_path(facility, total)`, and `approve` routes a `>= 2`-approver purchase through `pending_second_approval` instead of jumping to `approved` (mirrors the Orchestrator; threshold routing only). Two coupled gaps remain and **land together** when the auth layer is wired (now buildable atop `utils/auth.get_caller`), because both need authenticated claims (a count/role from the body is not enforcement): **(b) distinct-approver (M1)** — `approve`/`submit_approval` still only *count* approvals; with identity bound, require the two approvals from DISTINCT authenticated `sub`s (persist `sub` as `approver_id`), so one person can't satisfy "2 approvers"; **(c) customer-endpoint tenant-scoping (D2)** — `/api/orders`, `/api/impact`, `/api/reorder`, `/api/sites/{id}/ship-to`, `/api/review-items/*`, and the run endpoints are global/ungated (scope by `Caller.company_id`; the `orders` table has no tenant key, and `known_parts`/`price_db`/`suppliers` are global by design — both flagged as sub-prerequisites). Do **not** implement (b)/(c) on body-supplied identity. See `PHASE_R_RESOLUTION.md`. |
