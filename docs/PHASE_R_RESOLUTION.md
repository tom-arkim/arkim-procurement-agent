...
| §4.5 masking (core.py) | `_stub_sourcing` advanced a sourcing FAILURE to `COMPARISON` (masking) instead of `Phase.ERROR` | LOW | ✅ | `fix(orchestrator): route sourcing failure to Phase.ERROR` — `_stub_sourcing` now routes a failure to `Phase.ERROR` (matching the FastAPI path); a genuine empty-but-successful search still → `COMPARISON`. Masking pattern **gone from the codebase entirely** (not just "moot for the product"). Test-first: `test_sourcing_failure_advances_to_error` + `test_sourcing_empty_but_successful_stays_comparison`. CLEANUP §4.5 marked RESOLVED IN CODE. |
| Orchestrator fate | `core.py` is tested-but-not-on-the-shipping-path post-Streamlit | — | 🔒 DECISION-PENDING | Keep as the approval-routing reference / wire FastAPI onto it / retire it + `test_orchestrator*.py`. Memo with the analysis + recommendation: `docs/ORCHESTRATOR_DECISION.md` (recommends keep-as-staging-ground, fold "wire it in" into the auth/H1 cluster, do **not** retire). Now decoupled from §4.5 (resolved above). |
